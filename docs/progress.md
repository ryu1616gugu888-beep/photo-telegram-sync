...
## 2026-09-06(さらに続き): 動画2本目のドラフトも完成

- 動画1と同じ手法(実際のアプリ静止画+オリジナルイラストをffmpegでKen Burns風に結合)で、
  動画2「2万枚の写真を移行した話」のドラフト(25秒、無音・字幕なし)も完成。
  → `marketing/videos/drafts/video2_2万枚移行の裏話_draft.mp4`
- 新規に「送り方のコツ」スライド(原本画質保持・動画劣化なし・撮影日時引き継ぎ、SVGアイコン付き)を作成。
  実画面(同期・初期設定)とGitHub告知スライドは動画1と共通素材を再利用しブランド一貫性を確保。
- 短編動画2本とも「素材組み立て版ドラフト」が完成。ナレーション・字幕・BGMの追加が残作業。

## 2026-09-06(さらに続き): ナレーション・字幕を2本とも追加

- macOS標準の音声合成(`say -v Kyoko`)でナレーション音声を生成、各シーンの尺をナレーション長に
  合わせて再調整し、半透明バー+白文字の字幕を焼き込んだ「ナレーション・字幕付き版」を2本とも完成。
  → `video1_storage_anxiety_narrated.mp4`(約28秒)、`video2_2万枚移行の裏話_narrated.mp4`(約25秒)
- 開発者プライバシー部分・GitHub告知部分は両動画で同一の音声・字幕素材を再利用し、シリーズとしての
  統一感を確保。
- **BGMは未対応**: 著作権上、こちらでロイヤリティフリー音源を勝手に取得することは避けた。
  ユーザー側でロイヤリティフリーのBGMを選定し、上記動画に重ねてもらう必要がある。
- 次のアクション: BGM追加(ユーザー側作業)、Simulatorのタップ入力不具合の原因調査、
  Live Photoの実機検証。

## 2026-09-06(さらに続き): Simulatorタップ不具合の調査(未解決)

「今すぐ同期」ボタンがCloud Code iOS Simulatorツールからのタップに反応しない事象を系統的に調査した。

- **切り分け1**: ホーム画面のアプリアイコン・システムの写真ライブラリ権限ダイアログのボタンは
  問題なくタップに反応する → Simulatorのタップ機構自体は正常。
- **切り分け2**: `launchctl list`でアプリのプロセスIDを確認 → タップ前後でPIDが変わらず、
  クラッシュしていないことを確認。
- **切り分け3**: ContentViewに追加していたUI(容量無制限の説明ボックス)を一時的に削除して
  再ビルド・再テスト → **それでも再現**。UI追加が原因ではないと判明。
- **切り分け4**: 全く新規のSimulatorデバイス(`PTS-Diag-Test`)を作成し、同じアプリをインストール
  → **それでも再現**。特定デバイスの劣化状態が原因ではないと判明。
- CoreSimulatorServiceの強制再起動、デバイスの完全シャットダウン→再起動、アプリの完全アンインストール
  →再インストールも試したが、いずれも解消せず。
- **結論**: 根本原因は特定できなかった。UIコードの変更・デバイス固有の問題ではないことは切り分けで
  否定できたが、それ以上の原因(Claude Code iOS SimulatorツールとPhotoKit権限フローの組み合わせに
  起因する何か、等)は未特定のまま。診断用デバイスは削除済み、ContentView.swiftは正常な状態に復元済み。
- 次に試すべきこと: 実機での動作確認(TestFlightなしでもXcodeから直接インストール可能)、
  またはXcode GUIから直接Runしてこの問題が再現するか確認(このツール経由のビルド・インストールを
  経由しない場合との比較)。

## 2026-09-06(さらに続き): 認知拡大の方針転換とBGM追加

- ユーザーからの追加情報: SNSアカウントのフォロワーはごく少数、YouTube等の既存チャネルもなし。
  → フォロワー依存の拡散ではなく、SEO(note継続刊行+Zenn/Qiita)・プラットフォーム主導の発見
  (Hacker News/Reddit/GitHub Topics)・TikTokのアルゴリズムを優先する方針を`business-plan.md`に
  追記(6-4節)。
- 短編動画2本にBGMを追加。**外部音源は一切使わず、ffmpegのサイン波合成でオリジナルのアンビエント
  パッド(著作権フリー)をゼロから生成**し、ナレーション・字幕と合成。
...
## 2026-09-08: DIY版ハウツー記事(アプリ不使用)のドラフト作成

- ユーザー指示: アプリの話とは切り離し、「自分でiPhoneの写真をTelegramに同期したい場合、
  console等でどうやるか」を初心者向けに丁寧に書いた記事が欲しいとの依頼。
- **note記事(新規・アプリ非言及)**: 「iPhoneの写真をTelegramに自動バックアップする方法
  (無料・アプリ不要・Mac+Pythonで実現)」→ `marketing/articles/diy-manual-telegram-photo-backup.md`。
  前提条件(Mac必須・Apple ID共通・Telegramアカウント)を明記した上で、
  ①BotFatherでBot作成 → ②`getUpdates`でchat_id取得 → ③iCloud写真でMac同期設定 →
  ④`osxphotos`(OSS)でPHフォトライブラリを差分書き出し+Pythonスクリプトで自動送信
  (FloodWait/429リトライ・50MB超スキップ処理込み) → ⑤`cron`での定期自動実行、
  という5ステップ構成。この記事内では**アプリ開発の話・GitHubリンクは意図的に一切含めない**
  (他のnote/Zenn記事とは独立した純粋なハウツーとして切り出す方針)。
- 下書きのみ、実際の投稿はユーザー承認後。

## 2026-09-08(続き): DIYハウツー記事をnoteに公開

- ユーザーから「この記事のnoteを公開してください」と明示的な許可を得て、上記のDIYハウツー記事を
  実際にnote.comへ公開した。https://note.com/mute_cedum909/n/n9b49dc604901
- ハッシュタグ: #写真 #Telegram #iCloud #iPhone #Python #Mac #バックアップ。記事タイプは無料。
- 公開作業中、Pythonスクリプトのコードブロックに空行(ダブル改行)を入れるとnoteのブロックエディタが
  コードブロックを強制終了してしまう挙動を発見。誤って本文の一部が意図しない位置に挿入される事故が
  2回発生したが、いずれもその場で選択・削除・再入力により修正し、最終的な本文は問題なく完成。
  (今後note.comでコードブロックを扱う際は、空行を入れずに1行ずつ`\n`で繋ぐこと)
- 残り2本(note記事2本目・Zenn記事)は引き続き下書きのまま、投稿はユーザー承認後。

## 2026-09-08(続き): note記事2本目・Zenn記事は投稿見送りに決定

- ユーザーより「note記事、zenn技術記事は投稿の予定がありません」と明示。
  `why-github-not-appstore.md`(note2本目)・`zenn-technical-deep-dive.md`(Zenn)は
  下書きのままリポジトリに残すが、今後のアクション候補からは外す(気が変わった場合のみ再検討)。
- 次のアクションは、短編動画2本のTikTok投稿、GitHub Topicsタグ付け・Hacker News/Reddit等への
  投稿検討、Live Photo実機検証、Simulatorバグの実機確認、の4点に絞られる。
